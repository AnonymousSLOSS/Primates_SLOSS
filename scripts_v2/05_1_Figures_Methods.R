...
long_table <- read.csv("long_table.csv", header = TRUE)
...
DC_Din <- ggplot(long_table, aes(fill = Category, y=Value, x = 1)) +
  ...
  theme(axis.title.x=element_blank(),
        axis.title.y=element_text(size = 40, margin = margin(r = 100)),
...
long_table_IUCN <- read.csv("long_table_IUCN.csv", header = TRUE)

#Factorise the categories in order desired for plotting
long_table_IUCN$Category <- factor(long_table_IUCN$Category, levels = c("SL_better_perc","Equal_perc", "SS_better_perc"))

long_table_IUCN <- long_table_IUCN %>% 
  mutate(Category = factor(Category, levels = c("SL_better_perc","Equal_perc", "SS_better_perc")),
