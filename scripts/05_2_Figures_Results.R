...
full_hist_PA <- ggplot(patch_comb_stats_Din, aes(x = mean_patch_area_log, fill = factor(Occurrence))) +
  ...
  theme(legend.title = element_blank(),
        ...
        axis.text.y = element_text(size = 14)) +  # Adjust margin for y-axis label
  # ylim(0, 8000) +
  geom_text(data = patch_comb_dist[6,], aes(x = -1.8, y = Inf, label = paste("Species =", Species, "\nDatasets =", Datasets, "\nCombinations =", Combinations)),
            vjust = 2.5, hjust = 0, size = 5, inherit.aes = FALSE, check_overlap = TRUE)

full_hist_patch_PA <- ggplot(primatepatches_Din, aes(x = patch_area_log, fill = factor(Occurrence))) +
  geom_histogram(position = "stack", color = "black", linewidth = 0.3) +   # Use position = "stack" to stack bars
  xlab("Patch Size (ha)") + 
...
dev.new()
ggarrange(full_hist_patch_PA, full_hist_PA, ncol = 2, nrow = 1, labels = c("(a)", "(b)"), font.label = list(size = 18), hjust = c(-3.3,-3.6), vjust = c(1.8, 1.8), common.legend = TRUE, legend = "right")
...
plot_occ_Din <- ggplot(data = df_dummy, aes(x = x, y = y)) +
  ...
  annotate("text", x = 2.7, y = 20, label = "β = -3.18 (-1.56, -4.97)", 
           size = 8, family = "serif")
...
ggarrange(DC_Din, DC_IUCN_Din, widths = c(1.5,5), common.legend = TRUE, legend = "right", ncol = 3, nrow = 2, labels = "(c)", hjust = -0.1, vjust = 1.3, font.label = list(size = 45))
